...
def date_most_popular_tweets(df):
    temp = df.loc[df['Sentiment'] == 'Extremely Positive']
    s = temp['TweetAt'].value_counts().index.tolist()
    result = s[0]
    return result
...
def remove_non_alphabetic_chars(df):
    ...
    df['OriginalTweet'] = df['OriginalTweet'].replace('[^a-zA-Z ]', ' ', regex=True)
...
def count_words_with_repetitions(tdf):
    word_num = tdf['OriginalTweet'].str.len()
    count_word = word_num.sum()
    return count_word
...
def count_words_without_repetitions(tdf):
    ...
    for i in tdf['OriginalTweet']:
        total_word.extend(i)
    total_without_repetition = len(set(total_word))
    return total_without_repetition
...
def frequent_words(tdf, k):
    ...
    for j in range(len(tdf.index)):
        for w in tdf['OriginalTweet'][j]:
            terms.append(w)
    list = collections.Counter(terms).most_common(k)
    a = []
    for sublist in list:
        a.append(sublist[0])
    return a
...
def stemming(tdf):
    ps = PorterStemmer()
    tdf['OriginalTweet'] = tdf['OriginalTweet'].apply(lambda x: [ps.stem(item) for item in x])
    return tdf
...
def mnb_predict(df):
    x_train = np.array(df['OriginalTweet'])
    y = np.array(df['Sentiment'])
    cv = CountVectorizer(ngram_range=(0, 2), max_df=0.0001, min_df=0, stop_words='english')
...
def mnb_accuracy(y_pred, y_true):
    ...
    result = round(accuracy_score(y_pred, y_true), 3)
    return result
